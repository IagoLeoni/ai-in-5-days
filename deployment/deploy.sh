#!/usr/bin/env bash
#
# StackScribe deployment via the ADK CLI.
#
...
#   STACKSCRIBE_ENVIRONMENT  dev | staging | prod (default dev)
...
ENVIRONMENT="${STACKSCRIBE_ENVIRONMENT:-dev}"
...
python -m stack_scribe.evaluation.run_eval

case "$TARGET" in
  agent-engine)
    # Agent Engine is the managed runtime: it supplies the session service and
    # the Memory Bank that stack_scribe.memory.services binds to when
    # STACKSCRIBE_SESSION_BACKEND=vertex_ai.
...
set these on the calling service (the reasoning engine id is printed above):
  ...
  STACKSCRIBE_SESSION_BACKEND=vertex_ai
  STACKSCRIBE_AGENT_ENGINE_ID=<reasoning engine id>
...
EOF
  ...
  cloud-run)
    ...
    echo "  gcloud run services add-iam-policy-binding stackscribe-${ENVIRONMENT} \\"
