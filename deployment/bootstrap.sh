#!/usr/bin/env bash
#
# StackScribe one-command deployment.
#
...
TF_STATE_BUCKET="${TF_STATE_BUCKET:-${PROJECT_ID}-stackscribe-tfstate}"
...
if [[ -n "$CORPUS_BUCKET" ]]; then
  gcloud storage cp "${REPO_ROOT}"/stack_scribe/memory/corpus/*.json \
...
ok "unit tests pass"
"$PY" -m stack_scribe.evaluation.run_eval >/dev/null || die "evaluation suite failed - refusing to deploy"
...
export STACKSCRIBE_ENVIRONMENT="$ENVIRONMENT"
[[ -n "${SEARCH_DATASTORE:-}" ]] && export STACKSCRIBE_VERTEX_SEARCH_DATASTORE="$SEARCH_DATASTORE"
for role in PLANNER EDITORIAL RESEARCH EXTRACTION GUARDRAIL; do
  var="MODEL_${role}"
  [[ -n "${!var:-}" ]] && export "STACKSCRIBE_MODEL_${role}=${!var}"
done
...
if [[ "$DEPLOY_TARGET" == "agent-engine" ]]; then
  adk deploy agent_engine \
    --project "$PROJECT_ID" \
    --region "$REGION" \
    --display_name "StackScribe (${ENVIRONMENT})" \
    ...
    "${REPO_ROOT}/stack_scribe"
  ok "deployed to Agent Engine"
else
  adk deploy cloud_run \
    --project "$PROJECT_ID" \
    --region "$REGION" \
    --service_name "stackscribe-${ENVIRONMENT}" \
    --app_name stackscribe \
...
if [[ "$DEPLOY_TARGET" == "cloud-run" ]]; then
  SERVICE_URL="$(gcloud run services describe "stackscribe-${ENVIRONMENT}" \
...
if [[ -n "${GITHUB_REPO:-}" ]]; then
  ...
  ${DIM}(Settings -> Secrets and variables -> Actions -> Variables tab)${RESET}
    ...
    GCP_REGION                     = ${REGION}
    STACKSCRIBE_ENVIRONMENT       = ${ENVIRONMENT}
    STACKSCRIBE_DEPLOY_TARGET     = ${DEPLOY_TARGET}
  ...
  Or from the command line:
    ...
    gh variable set STACKSCRIBE_ENVIRONMENT --body "${ENVIRONMENT}" --repo ${GITHUB_REPO}
    gh variable set STACKSCRIBE_DEPLOY_TARGET --body "${DEPLOY_TARGET}" --repo ${GITHUB_REPO}
...
printf '\n%s%sStackScribe is deployed.%s  project=%s env=%s target=%s\n\n' \
