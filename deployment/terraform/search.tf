...
# (stack_scribe/memory/corpus/), which is what makes the repository runnable
...
resource "google_discovery_engine_data_store" "brand_kb" {
  ...
  location          = "global"
  data_store_id     = "stackscribe-${var.environment}-brand-kb"
  display_name      = "StackScribe brand knowledge base (${var.environment})"
...
resource "google_discovery_engine_search_engine" "brand_kb" {
  project        = var.project_id
  engine_id      = "stackscribe-${var.environment}-brand-kb-engine"
  collection_id  = "default_collection"
  location       = google_discovery_engine_data_store.brand_kb.location
  display_name   = "StackScribe brand knowledge search (${var.environment})"
...
# stack_scribe/memory/corpus/) means the index can always be rebuilt.
###############################################################################

resource "google_storage_bucket" "corpus" {
  project                     = var.project_id
  name                        = "${var.project_id}-stackscribe-${var.environment}-corpus"
  location                    = var.region
