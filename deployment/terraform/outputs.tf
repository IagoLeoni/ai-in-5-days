output "service_url" {
  description = "URL of the deployed StackScribe Cloud Run service."
...
output "search_datastore_name" {
  description = "Set as STACKSCRIBE_VERTEX_SEARCH_DATASTORE to enable Vertex AI Search retrieval."
  ...
  value       = <<-EOT
    1. Populate secret values and create the database user (never in Terraform,
       which would persist them in state):
         ...
         gcloud sql users create stackscribe_agent \
           --instance=${google_sql_database_instance.sessions.name} --password="$DB_PASSWORD"
    2. Stage and import the brand corpus:
         gsutil -m cp stack_scribe/memory/corpus/*.json gs://${google_storage_bucket.corpus.name}/
