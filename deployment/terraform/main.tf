###############################################################################
# StackScribe - core infrastructure
#
...
locals {
  service_name = "stackscribe-${var.environment}"

  common_labels = {
    application = "stackscribe"
...
resource "google_service_account" "agent_runtime" {
  project      = var.project_id
  account_id   = "stackscribe-${var.environment}-sa"
  display_name = "StackScribe agent runtime (${var.environment})"
  description  = "Runtime identity for the StackScribe Cloud Run service. Least privilege; secret access granted per-secret."
}
...
resource "google_artifact_registry_repository" "agent_images" {
  project       = var.project_id
  location      = var.region
  repository_id = "stackscribe"
  format        = "DOCKER"
  description   = "StackScribe agent container images"
...
resource "google_cloud_run_v2_service" "agent" {
  ...
  template {
    ...
    containers {
      ...
      env {
        name  = "STACKSCRIBE_ENVIRONMENT"
        value = var.environment
      }
      env {
        name  = "STACKSCRIBE_ENABLE_CLOUD_TRACE"
        value = "1"
      }
      env {
        name  = "STACKSCRIBE_ENABLE_DLP_REDACTION"
        value = var.environment == "prod" ? "1" : "0"
      }
      env {
        name  = "STACKSCRIBE_SESSION_BACKEND"
        value = "database"
      }
      ...
      env {
        name = "STACKSCRIBE_DATABASE_URL"
        value = join("", [
          "postgresql+pg8000://stackscribe_agent:{db_password}@/",
      ...
      env {
        name  = "STACKSCRIBE_REQUIRE_PUBLISH_CONFIRMATION"
        value = "1" # the human-in-the-loop gate is not configurable in deployment
      }
      env {
        name  = "STACKSCRIBE_VERTEX_SEARCH_DATASTORE"
      ...
      env {
        name  = "STACKSCRIBE_CMS_API_TOKEN_SECRET"
        value = "${google_secret_manager_secret.cms_api_token.id}/versions/latest"
      }
      env {
        name  = "STACKSCRIBE_DB_PASSWORD_SECRET"
...
resource "google_monitoring_alert_policy" "publish_rejections" {
  ...
  project      = var.project_id
  display_name = "StackScribe (${var.environment}): human rejected a publish"
  combiner     = "OR"
...
resource "google_monitoring_alert_policy" "guardrail_blocks" {
  ...
  project      = var.project_id
  display_name = "StackScribe (${var.environment}): guardrail blocked an action"
  combiner     = "OR"
