...
#   printf '%s' "$DB_PASSWORD" | gcloud secrets versions add stackscribe-dev-db-password --data-file=-
###############################################################################

resource "google_secret_manager_secret" "cms_api_token" {
  project   = var.project_id
  secret_id = "stackscribe-${var.environment}-cms-api-token"
...
resource "google_secret_manager_secret" "db_password" {
  project   = var.project_id
  secret_id = "stackscribe-${var.environment}-db-password"
  labels    = local.common_labels
...
resource "google_secret_manager_secret" "gemini_api_key" {
  ...
  project   = var.project_id
  secret_id = "stackscribe-${var.environment}-gemini-api-key"
  labels    = local.common_labels
