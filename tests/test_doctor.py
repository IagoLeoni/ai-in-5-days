...
from stack_scribe.doctor import Check, run_checks
...
def test_doctor_blocks_only_on_missing_credentials(monkeypatch):
    monkeypatch.setenv("STACKSCRIBE_PROJECT_ID", "")
    checks = run_checks()
...
def test_doctor_flags_an_unrecognised_model_override(monkeypatch):
    monkeypatch.setenv("STACKSCRIBE_MODEL_PLANNER", "gemini-9.9-imaginary")
...
def test_doctor_warns_when_the_publish_gate_is_disabled(monkeypatch):
    monkeypatch.setenv("STACKSCRIBE_REQUIRE_PUBLISH_CONFIRMATION", "0")
...
def _run_doctor_in(cwd, extra_env: dict[str, str] | None = None):
    """Run `python -m stack_scribe.doctor` in `cwd` with a scrubbed environment."""
    import os
    import subprocess
    import sys
    ...
    return subprocess.run(
        [sys.executable, "-m", "stack_scribe.doctor"],
        cwd=str(cwd),
        env=env,
        capture_output=True,
        text=True,
...
def test_api_key_in_dotenv_is_picked_up(tmp_path):
    ...
    pydantic-settings reads `.env` only for its own STACKSCRIBE_ fields and does
