...
from stack_scribe.agents.pipeline import (
    MAX_REVISION_ROUNDS,
    build_content_planning_pipeline,
    build_root_agent,
)
from stack_scribe.models import DEFAULT_MODEL_ROUTES, resolve_model, routing_table
from stack_scribe.schemas import ContentPlan, DraftCritique, PublishRequest
...
def test_mechanical_roles_route_to_the_cheapest_tier():
    ...
    from stack_scribe.models import MODEL_PRICING_USD_PER_1M
...
def test_all_default_routes_are_known_ga_models():
    ...
    from stack_scribe.models import KNOWN_MODELS
...
def test_routes_are_overridable_without_a_code_change(monkeypatch):
    monkeypatch.setenv("STACKSCRIBE_MODEL_RESEARCH", "gemini-2.5-pro")
...
def test_prod_refuses_to_disable_the_publish_gate():
    ...
    from stack_scribe.config import Settings
...
def test_missing_required_secret_raises_an_actionable_error():
    from stack_scribe.config import SecretResolutionError, resolve_secret

    with pytest.raises(SecretResolutionError, match="STACKSCRIBE"):
        resolve_secret("", required=True)


def test_optional_secret_degrades_to_none():
    from stack_scribe.config import resolve_secret
...
def test_sqlite_url_needs_no_password_substitution():
    ...
    settings = Settings(database_url="sqlite+aiosqlite:///./.stackscribe/sessions.db")
...
def test_database_url_placeholder_is_filled_from_secret_manager(monkeypatch):
    ...
    import stack_scribe.config as config_module
...
def test_missing_db_password_secret_fails_loudly():
    ...
    from stack_scribe.config import SecretResolutionError, Settings
...
def test_no_secret_literals_in_source():
    ...
    package = Path(__file__).parent.parent / "stack_scribe"
