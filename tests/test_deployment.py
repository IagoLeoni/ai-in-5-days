...
def test_deploy_workflow_runs_gates_before_deploying():
    ...
    assert body.index("stackscribe-eval") < body.index("adk deploy")
...
def test_no_deprecated_vertexai_sdk_imports():
    ...
    StackScribe reaches models through ADK on the current google-genai SDK.
    ...
    for path in (REPO_ROOT / "stack_scribe").rglob("*.py"):
