...
> **Why `GOOGLE_GENAI_USE_VERTEXAI=0` is optional but explicit here.** With an API key present and the flag unset, StackScribe selects API-key mode on its own. Setting it to `0` makes the choice unambiguous, which matters if you also happen to have `gcloud` credentials on the machine — otherwise those could win.
...
Run it before anything else. It reports the credential mode, the effective model routing, and — the useful part — **which subsystems are running degraded**, because StackScribe is built to degrade gracefully and a clean startup won't tell you which mode you're in.

```
StackScribe preflight
  environment: local
  credentials: api_key
  ...
  ✓ Session state          SQLite at ./.stackscribe/sessions.db - durable across restarts
  ~ Long-term memory       In-process - author preferences are not recalled in a later session
      → Set STACKSCRIBE_AGENT_ENGINE_ID to enable Memory Bank (deployed only)
...
If you hit rate limits, lower `MAX_REVISION_ROUNDS` in `stack_scribe/agents/pipeline.py`, or route more roles to Flash-Lite in `.env`.
...
docker run --rm -p 8080:8080 -e GOOGLE_API_KEY="$GOOGLE_API_KEY" \
  -e GOOGLE_GENAI_USE_VERTEXAI=0 stackscribe:local
```
