"""StackScribe application entry point.

This module is what ``adk run stack_scribe``, ``adk web`` and
``adk deploy`` load. It assembles the pieces built elsewhere in the package:

* the agent tree                  -> :mod:`stack_scribe.agents.pipeline`
* the plugins                     -> :mod:`stack_scribe.plugins`
* history compaction              -> :func:`stack_scribe.memory.services.build_compaction_config`
* persistent sessions and memory  -> :mod:`stack_scribe.memory.services`
* logging and tracing             -> :mod:`stack_scribe.observability`
...
from stack_scribe.config import get_settings
...
from stack_scribe.models import routing_table
from stack_scribe.observability.logging_config import configure_logging, get_logger
from stack_scribe.observability.tracing import configure_tracing
from stack_scribe.plugins import AsyncMemoryPlugin, GuardrailPlugin, IntentOutcomePlugin
...
def build_app() -> App:
    """Assemble the complete StackScribe application.
