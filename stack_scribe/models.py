...
StackScribe is unaffected by the one migration that *does* bite: the legacy
...
planner and critic do. Promote those two roles without touching code::
    ...
    export STACKSCRIBE_MODEL_PLANNER=gemini-3.6-flash
    export STACKSCRIBE_MODEL_EDITORIAL=gemini-3.6-flash
...
#: `STACKSCRIBE_MODEL_*` override before it becomes a 404 mid-conversation.
...
def resolve_model(role: ModelRole) -> str:
    ...
    Returns:
        ...
        override (``STACKSCRIBE_MODEL_<ROLE>`` or ``STACKSCRIBE_MODEL_<ROLE>``)
        takes precedence over the default, and is returned even if unrecognised.
    ...
    return (
        os.getenv(f"STACKSCRIBE_MODEL_{role.upper()}")
        or os.getenv(f"STACKSCRIBE_MODEL_{role.upper()}")
        or DEFAULT_MODEL_ROUTES[role]
    )
