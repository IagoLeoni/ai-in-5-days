...
from stack_scribe.errors import ErrorCode, tool_error
...
from stack_scribe.observability.tracing import get_tracer, set_span_attributes
...
class IntentOutcomePlugin(BasePlugin):
    ...
    converted into a guided error envelope (see :mod:`stack_scribe.errors`), so
    ...
    async def before_tool_callback(
        ...
        :class:`~stack_scribe.plugins.guardrail_plugin.GuardrailPlugin`.)
        """
        ...
        span_cm = get_tracer().start_as_current_span(
            f"stackscribe.tool.{tool.name}", kind=SpanKind.INTERNAL
        )
        span = span_cm.__enter__()
        set_span_attributes(
            span,
            {
                "stackscribe.decision_id": decision_id,
                "stackscribe.tool.name": tool.name,
                "stackscribe.agent.name": tool_context.agent_name,
                "stackscribe.invocation_id": tool_context.invocation_id,
                "stackscribe.tool.args": _summarise(tool_args),
            },
        )
    ...
    async def after_tool_callback(
        ...
        *,
        ...
        if span is not None:
            set_span_attributes(span, {"stackscribe.tool.status": status})
        return None
    ...
    def _close_span(
        ...
        try:
            set_span_attributes(
                span,
                {
                    "stackscribe.tool.duration_ms": duration_ms,
                    "stackscribe.tool.result_status": result_status,
                },
            )
