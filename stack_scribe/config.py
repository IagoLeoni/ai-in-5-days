"""Centralised, validated configuration for StackScribe.
...
def _load_dotenv_into_environment() -> None:
    ...
    * :class:`Settings` below is a pydantic-settings model, so it reads `.env`
      itself - but **only for its own ``STACKSCRIBE_``-prefixed fields**.
...
class Settings(BaseSettings):
    ...
    # --- Google Cloud -------------------------------------------------------
    # These two use the Google-standard names (no STACKSCRIBE_ prefix) because
    ...
    database_url: str = "sqlite+aiosqlite:///./.stackscribe/sessions.db"
    agent_engine_id: str = ""
    ...
    def _validate_deployment_invariants(self) -> Settings:
        ...
        if self.session_backend == "vertex_ai" and not self.agent_engine_id:
            raise ValueError(
                "session_backend='vertex_ai' requires STACKSCRIBE_AGENT_ENGINE_ID. "
                "Provision one with `terraform apply` in deployment/terraform, or "
                "switch to STACKSCRIBE_SESSION_BACKEND=database."
            )
        if self.environment == "prod":
            if not self.project_id:
                raise ValueError(
                    "GOOGLE_CLOUD_PROJECT must be set when STACKSCRIBE_ENVIRONMENT=prod."
                )
    ...
    def resolved_database_url(self) -> str:
        ...
        Raises:
            SecretResolutionError: When the URL needs a password but
                ``STACKSCRIBE_DB_PASSWORD_SECRET`` is unset or unreadable.
    ...
    if not secret_resource_name:
        if required:
            raise SecretResolutionError(
                ...
                "STACKSCRIBE_*_SECRET environment variable to a Secret Manager "
