...
1. **Installs a real ``TracerProvider``** with a service-identifying resource, so
   ...
   ``STACKSCRIBE_ENABLE_CLOUD_TRACE=1``, to an OTLP collector when
...
from stack_scribe.observability.redaction import redact_text

logger = logging.getLogger(__name__)

_TRACING_CONFIGURED = False
_TRACER_NAME = "stack_scribe"
...
def configure_tracing(force: bool = False) -> None:
    ...
    1. ``STACKSCRIBE_ENABLE_CLOUD_TRACE=1`` -> Google Cloud Trace (batched).
...
def get_tracer() -> trace.Tracer:
    """Return the StackScribe tracer, configuring the provider on first use."""
    configure_tracing()
...
def set_span_attributes(span: Span, attributes: dict[str, Any]) -> None:
    ...
    All string values pass through :func:`~stack_scribe.observability.redaction.redact_text`
...
def pipeline_span(
    ...
    Args:
        name: Span name, e.g. ``"stackscribe.seo.score"``. Use dotted,
    ...
    Example:
        >>> with pipeline_span("stackscribe.research.gather", subtopic=topic) as span:
