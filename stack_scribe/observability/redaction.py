...
(:mod:`stack_scribe.observability.logging_config`), the span attribute setter
(:mod:`stack_scribe.observability.tracing`), and the async memory writer
(:mod:`stack_scribe.plugins.memory_plugin`).
...
* **Optionally on:** Google Cloud DLP for the classes regex cannot catch
  ...
  ``STACKSCRIBE_ENABLE_DLP_REDACTION=1``. DLP runs *in addition to*, never
...
def redact_text(value: str) -> str:
    ...
    ``STACKSCRIBE_ENABLE_DLP_REDACTION=1``.
