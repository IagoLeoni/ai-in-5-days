...
StackScribe degrades gracefully by design: no Vertex AI Search means the
...
from stack_scribe.config import Settings, get_settings
from stack_scribe.models import KNOWN_MODELS, routing_table
    ...
    if backend == "vertex_ai":
        if not settings.agent_engine_id:
            return Check(
                "Session state",
                "missing",
                "vertex_ai backend selected but STACKSCRIBE_AGENT_ENGINE_ID is unset",
                "Set it, or use STACKSCRIBE_SESSION_BACKEND=database",
            )
...
def _check_cms() -> Check:
    ...
    return Check(
        "CMS",
        "ok",
        "Draft-only mode - approved posts go to .stackscribe/publish_outbox.jsonl "
