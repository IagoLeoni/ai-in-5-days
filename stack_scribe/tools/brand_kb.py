...
``STACKSCRIBE_VERTEX_SEARCH_DATASTORE`` is configured, and by a bundled local
corpus otherwise, so the agent is fully runnable without a GCP project. The
selection happens in :mod:`stack_scribe.memory.vector_store`; these tools are
...
from stack_scribe.schemas import (
...
def retrieve_brand_style_guide(topic: str, content_type: str = "blog_post") -> dict[str, Any]:
    ...
    Returns:
        On success, a dict matching :class:`~stack_scribe.schemas.BrandStyleGuide`:
...
def search_published_posts_for_overlap(
    ...
    Returns:
        ...
        :class:`~stack_scribe.schemas.PriorPostSearchResult`:
