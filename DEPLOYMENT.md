# Deploying StackScribe to Google Cloud
...
GCP_SERVICE_ACCOUNT            = stackscribe-dev-deploy@....iam.gserviceaccount.com
GCP_PROJECT_ID                 = my-gcp-project
GCP_REGION                     = us-central1
STACKSCRIBE_ENVIRONMENT       = dev
STACKSCRIBE_DEPLOY_TARGET     = agent-engine
```
...
**Agent Engine** (default) — the managed runtime on the Gemini Enterprise Agent Platform. It supplies the session service and Memory Bank that `stack_scribe/memory/services.py` binds to when `STACKSCRIBE_SESSION_BACKEND=vertex_ai`. Least operational work; you don't manage a container or a database.
...
Defaults, from `stack_scribe/models.py`:
...
The one migration that does bite is the legacy `vertexai.generative_models` / `vertexai.language_models` / `vertexai.caching` modules, which stop working on **24 June 2026**. StackScribe reaches models exclusively through ADK on the current `google-genai` SDK and imports none of them, so there is nothing to migrate. There is a test asserting this stays true.
...
**Cloud Run:**
```bash
SERVICE_URL=$(gcloud run services describe stackscribe-dev --region us-central1 --format='value(status.url)')
